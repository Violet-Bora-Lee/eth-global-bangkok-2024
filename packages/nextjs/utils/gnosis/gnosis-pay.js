...
export function isDelayModule(bytecode) {
  return getAddress(getGenericProxyMastercopy(bytecode)) === getAddress(DELAY_MOD_MASTERCOPY);
}
...
export async function getDelayModule(address) {
  ...
  const delayMod = modules.find(module => isDelayModule(module.bytecode))?.address;
